...
└── config.nix         # Runtime user identity (gitignored, auto-generated)
```
...
- **`config.nix`** is gitignored + auto-generated by `just build` from env vars.
...
- **Impure `--impure` flag** — required due to `config.nix` generation. Acceptable tradeoff for portability.
...
- `config.nix` is written at runtime by `just build` — if missing, run `just build`.
